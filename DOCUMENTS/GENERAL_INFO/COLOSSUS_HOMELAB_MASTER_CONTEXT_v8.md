...
**Last Updated:** 2026-02-26 (v0.3.2 deployed — externalized document storage via virtiofs, Dockerfile migrated to Ubuntu 24.04/Rust 1.88, --cpu host for ONNX/AVX2, PBS backup coverage for app VMs)
---
...
â””â”€â”€ legal-docs    compression=zstd (document storage for colossus-legal)
...
### 5.4 App VM Document Storage (virtiofs)

Application VMs (VM-120, VM-220) use virtiofs to mount externalized document storage from host ZFS datasets, following the same pattern as database VMs.
...
- Podman volume: `/mnt/data/legal-docs:rw` (via Ansible-managed Quadlet)
- SELinux: `context="system_u:object_r:container_file_t:s0"` on mount unit

**VM CPU requirement:** All app VMs running colossus-legal **must** use `--cpu host` due to ONNX Runtime requiring AVX2 instructions. Default Proxmox CPU type (`kvm64`) only provides SSE2, causing SIGILL (exit code 132) at container startup.

```bash
qm set <VMID> --cpu host
```

See `APP_VM_VIRTIOFS_STORAGE_RUNBOOK_v2.md` for complete operational procedures.

---


### 5.5 TrueNAS Storage (Secondary Backup + Shared Library)
...
## 14.8 App VM Document Storage & Build Pipeline (COMPLETE)

Completed 2026-02-24 through 2026-02-26 (v0.3.2):

1. ~~Create ZFS datasets for document storage (dev-zfs/legal-docs, prod-zfs/legal-docs)~~ ✅
2. ~~Create Proxmox directory mappings (dev-legal-docs, prod-legal-docs)~~ ✅
3. ~~Update Butane configs with virtiofs mount units + Semaphore SSH key~~ ✅
4. ~~Create PROD Butane/Ignition from scratch (colossus-prod-app1.bu/.ign)~~ ✅
5. ~~Destroy and recreate both app VMs with virtiofs attached~~ ✅
6. ~~Migrate Dockerfile from Debian Bookworm to Ubuntu 24.04~~ ✅
7. ~~Upgrade Rust toolchain to 1.88 (required by ort-sys/ONNX Runtime)~~ ✅
8. ~~Add `--locked` to cargo builds for reproducibility~~ ✅
9. ~~Add `--no-cache` and source repo check to build-release.sh~~ ✅
10. ~~Fix VM CPU type to `--cpu host` for AVX2 support~~ ✅
11. ~~Update 04-recreate-app-vm.sh with `--cpu host`~~ ✅
12. ~~Fix Ansible role (push to origin before Semaphore deploy)~~ ✅
13. ~~Deploy v0.3.2 to DEV and PROD via Semaphore~~ ✅
14. ~~Verify PDFs served via externalized storage on both environments~~ ✅
15. ~~Create comprehensive APP_VM_VIRTIOFS_STORAGE_RUNBOOK_v2.md~~ ✅
16. ~~Reboot test both VMs (mount, containers, health survive restart)~~ ✅
17. ~~Add PBS backup schedules for VM-120 and VM-220~~ ✅

**Key lessons:**
- Dockerfile base image must match glibc requirements of native dependencies (ort-sys needs glibc 2.38+)
- `cargo build --locked` prevents surprise crate upgrades that break builds
- `--no-cache` on container builds reveals hidden issues from stale layers
- VM CPU type `kvm64` lacks AVX2 — always use `--cpu host` for ONNX workloads
- Ignition is first-boot only — must destroy and recreate VMs to apply config changes
- Always `git push` before Semaphore deploys — Semaphore pulls from origin, not local working tree
...
### 15.4 Orphan Containers from Butane Deployment (RESOLVED)

The original Butane/Ignition configs for VM-120 and VM-220 used container names `colossus-legal-backend` and `colossus-legal-frontend`. The Ansible deploy-app role uses `colossus-backend` and `colossus-frontend`.

**Resolution (2026-02-25):** Both DEV and PROD Butane configs were updated to use the Ansible container names (`colossus-backend`/`colossus-frontend`) and retranspiled to Ignition. VMs were destroyed and recreated with the updated configs during the virtiofs storage work. No orphan containers remain.
...
- ~~Scheduled backups for all VMs/CTs~~ — **Done (2026-02-13, updated 2026-02-26):** All 9 VMs/CTs on daily PBS schedule
...
- ~~App VM document storage~~ — **Done (2026-02-26):** Externalized document storage via virtiofs, Dockerfile rebuilt for Ubuntu 24.04/Rust 1.88
...
| `04-recreate-app-vm.sh` | pve-1/pve-2 | Destroy + recreate app VM with virtiofs + `--cpu host` |
...
|----------|--------|---------|
| This document (Master Context v8) | **ACTIVE** | Canonical project reference |
...
| `APP_VM_VIRTIOFS_STORAGE_RUNBOOK_v2.md` | **ACTIVE** | App VM document storage + build pipeline procedures |
| `COLOSSUS_SERVICE_ENDPOINTS_v1.md` | **ACTIVE** | All service IPs, ports, URLs |
...
| `VIRTIOFS_PROD_BUILD_FIX_SESSION_TRANSITION.md` | **REFERENCE** | v0.3.2 virtiofs + build pipeline session record |
...
| `COLOSSUS_HOMELAB_MASTER_CONTEXT_v5.md` | Superseded by v6 |
| `COLOSSUS_HOMELAB_MASTER_CONTEXT_v6.md` | Superseded by v7 |
| `COLOSSUS_HOMELAB_MASTER_CONTEXT_v7.md` | Superseded by v8 |
| `COLOSSUS_HOMELAB_MASTER_CONTEXT.md` | Superseded by v3 |
| `APP_VM_VIRTIOFS_STORAGE_RUNBOOK_v1.md` | Superseded by v2 |
...
| Semaphore infra key | CT-315 + Workstation `~/.ssh/` | Passphrase-free, authorized on all 12 hosts |
...
|-------|-----|------------|-------|
| `ghcr.io/rhrywnak/colossus-backend` | v0.3.2, latest | Public | Rust/Axum, Ubuntu 24.04 base, Rust 1.88, ONNX Runtime (AVX2) |
| `ghcr.io/rhrywnak/colossus-frontend` | v0.3.2, latest | Public | React/nginx, runtime config.js via Ansible |

**Build details (backend Dockerfile):**
- Builder stage: `ubuntu:24.04` + Rust 1.88 via rustup (not official rust image)
- Runtime stage: `ubuntu:24.04` + `libssl3t64` + `ca-certificates` + `curl`
- Both `cargo build` lines use `--locked` for reproducible builds
- Build script (`build-release.sh`) uses `--no-cache` to prevent stale layers
...
**Note:** App VM backup jobs (backup-prod-app, backup-dev-app) were added 2026-02-26 after VMs were rebuilt with virtiofs storage. Initial baseline backups completed same day.
